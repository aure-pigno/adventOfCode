            ...
            while c:
                ...
                for j in range(len(tbl)):
                    for i in range(len(tbl[0])):
                        if tbl[j][i] < 10 or [j, i] in known_indexes:
                            continue
                        known_indexes.append([j, i])
                        for [y, x] in helper.flatten([[[j + dy, i + dx] for dx in range(-1,2)] for dy in range(-1,2)]):
                            if x != -1 and y != -1 and x != len(tbl[0]) and y != len(tbl):
                                tbl[y][x] += 1
                                if tbl[y][x] == 10 and [y, x] not in known_indexes:
                                    c = True
            if part != 1 and len(known_indexes) == 100:
                return loop + 1

            tbl = self.replace(tbl)
            count += len(known_indexes)
        return count
